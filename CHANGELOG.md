...
- Updating a multi-file mod from a source without file categories — every custom source (directory/manifest/api), and CurseForge whenever its release-type labels (release/beta/alpha) don't repeat across versions — no longer pairs the update's replacement file with a stored file by listing order, a coin flip that could retain the stale main file deployed beside its replacement; when categories decide nothing, the primary flag now pairs a new main with the old main and leaves unchanged extras alone (#144)

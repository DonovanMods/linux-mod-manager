...
func TestDoUpdateRollback_Locked_RefusesBeforeHeader_Text(t *testing.T) {
	...
	assert.Equal(t, "Rollback available: 2.0 → 1.0\nmod is locked: Mod One is locked at v2.0 in profile default - move the lock with 'lmm mod lock -s test-src -p default mod1 <version>' or unlock with 'lmm mod unlock -s test-src -p default mod1'\n", out)

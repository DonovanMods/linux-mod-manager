...
func doVerify(cmd *cobra.Command, svc *core.Service, game *domain.Game, args []string) error {
	...
	for key, expectedCount := range fileCountByMod {
		...
		if game.DeployMode == domain.DeployCompile && core.HasRetainedCompileSource(gameCache, game.ID, mod.SourceID, mod.ID, mod.Version, mod.FileIDs) {
			continue
		}
...
// sourceMappedMod is a thin CLI-local wrapper over core.SourceMappedMod
// (moved there in #224 Task 2 so the verify engine can use it too) kept so
// every existing call site in this package needs no change. A later task
// (#224 Task 7) deletes this wrapper once the CLI calls the engine
// directly instead of duplicating its own verify logic.
func sourceMappedMod(game *domain.Game, mod *domain.Mod) *domain.Mod {
	return core.SourceMappedMod(game, mod)
}

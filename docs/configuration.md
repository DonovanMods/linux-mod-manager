...
| `disabled`  | bool   | Optional. The mod stays in the profile — keeping its load-order position and its pinned version — but is switched off: no deploy, and no converge run re-enables it. Written by `lmm mod disable`; cleared by `lmm mod enable` and by an explicit `lmm install` of the same mod; omitted when false. |

`disabled` is the profile's own record of the off state, not just the database's. **A missing `disabled` key means enabled**, so profile files written by earlier versions of lmm are read exactly as before. Mods that were already disabled when the key arrived are the one exception: their off state lived only in the database, so the first `lmm` command after that upgrade records each one in the profile that lists it (once per installation, and it says how many it recorded). It survives `lmm profile export`/`import`, `lmm profile sync`, `lmm profile switch`, `lmm profile apply` and a rebuilt database; a mod newly added to a profile still defaults to enabled. Setting it by hand is supported: the next `lmm profile apply` undeploys the mod and clears its enabled flag, and `lmm deploy` (and the web UI's Deploy button) skips it, while `lmm deploy --mod <mod-id>` refuses it by name — `lmm deploy --all`, which exists to deploy switched-off mods, still deploys it.

Two commands clear the marker, and both are an explicit statement about that one mod: `lmm mod enable` (which clears it even when the database already said the mod was enabled, so a document and a database that disagree are recoverable), and `lmm install` of the same mod — asking for a mod by name is a request to have it, so the install does not leave a mod behind that the next converge run would switch straight back off. Enabling a mod whose files were never downloaded (an imported profile on a fresh machine) needs that download — `lmm install --id <mod-id>` — because a disabled mod is never fetched.

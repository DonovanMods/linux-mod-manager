...
| `is_default`  | bool   | Whether this is the game's active profile — the one `lmm profile switch` last made active, whose mods are in the game directory. lmm keeps exactly one profile per game marked, and marks a game's first profile when it creates it; `lmm profile list` warns when a hand edit leaves none or several, or leaves a profile file lmm cannot read, and until that is fixed lmm refuses to deploy or purge that game's files rather than guess which profile is active (`lmm profile switch <name>` then only marks `<name>`). A game with a single profile file counts it as active, marked or not                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

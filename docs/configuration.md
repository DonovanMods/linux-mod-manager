...
| `is_default`  | bool   | Whether this is the game's active profile — the one `lmm profile switch` last made active, whose mods are in the game directory. lmm keeps exactly one profile per game marked; `lmm profile list` warns when a hand edit leaves none or several                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
The upgrade step edits only the entries it records. Comments, key order, flow style, indentation, line endings (LF, CRLF or CR) and `~/` paths stay as you wrote them. It writes to the file the profile was read from, and through a symlink it writes to the link's target. If it cannot edit a file, a warning names that file, every other profile is still recorded, and lmm tries that file again once it changes. A file that was read-only is tried again once lmm itself has changed it (switching away from that profile is enough). An entry laid out in a way the step cannot change keeps that layout through lmm's own changes, so edit the entry, or run `lmm mod disable` for the mod, which records it (rewriting the file whole, with a `.bak` copy, if the layout still does not allow an in-place edit). That happens when:

...
- **Reading mod paks is solved.** `internal/unrealpak.Reader` enumerates and
  ...
  prebuilt mod pak inspected declared an *empty* CompressionMethods table
...
- **No diff-derive code exists.** The entire pipeline is diff-*apply*
...
- **Known hazards:**
  - Prebuilt tables are *stale whole-table snapshots* (measured: 167 rows in
...
2. **Differ fidelity** — does a `Name`-keyed row differ against the *live*
...
4. **Asset probe** — does the inferred asset layout hold in 1–2 real
   ...
   (`.uasset`/`.uexp`)? *Probe + report only; no asset conversion prototype
   unless it turns out trivial.*

...
| 2026-08-29 | Ruling 7: `profile import`'s install prompt moves before Apply (the profile is no longer saved before the question); install/import conflict prompts stay where they were, but a re-run Apply may print its download line as cached | Ruling 1's Plan/Apply split needs a fixed, recorded prompt order |

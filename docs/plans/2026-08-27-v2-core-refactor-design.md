...
| 2026-08-29 | Ruling 7: `profile import`'s install prompt moves before Apply (the profile is no longer saved before the question); install/import conflict prompts stay where they were, but a re-run Apply may print its download line as cached (the second clause superseded 2026-08-29 by the Apply-order row below; the `import <archive>` half superseded 2026-08-30 by Ruling 18 below) | Ruling 1's Plan/Apply split needs a fixed, recorded prompt order |
...
| 2026-08-30 | Ruling 16 (#305, task-18 review fix wave): the DB→profile pair is a two-step commit and cancellation is adjudicated by sub-class. **(A) Completing writes** — a profile write that completes an already-applied DB mutation (`uninstall.go:287`, `purge.go:138`, `install.go:1823`/`:2279`, `import_archive.go:367`, `adopt.go:617`, `profile_import.go:433`, `mod_edit.go:332`+`:338` (the relink's two profile legs) and `:374` (the version-only edit), `switch.go:474`) runs through the shared `core.completeProfileWrite` helper, which executes it under `context.WithoutCancel(ctx)` so it always finishes, then re-checks `ctx.Err()` and lets the cancellation outrank the write's own error; the mirrored `core.completeDBWrite` (`ops.go:93`) does the identical thing in the OTHER direction — a DB write completing an already-applied profile mutation — for the relink's third leg, `mod_edit.go:343` (`saveInstalledMod` the new DB row under the new identity). A relink is therefore a three-step completion chain, not a pair: `mod_edit.go:319`'s DB delete of the OLD row, then `:332` `RemoveMod` (drop the old ref) → `:338` `UpsertMod` (write the new ref) → `:343` `saveInstalledMod` (the new row), with the cancellation re-checked once, after all three, at `:348`; the version-only edit's OWN plain `saveInstalledMod` call (`mod_edit.go:363`) is now guarded `if !plan.Relink`, so a relink never redundantly re-saves the row its own chain's `:343` leg already wrote (corrected 2026-08-30, re-review round 2 NEW-2: this row previously described the relink as a two-step, profile-only completion — accurate when the row was written at `3bf6afa`, but fix wave round 1b, `57dfcfa`, landed the same day, added the DB-save leg plus `completeDBWrite`, and introduced the `!plan.Relink` guard; neither doc was revisited). Each caller returns its partial result with `context.Canceled` immediately after, so the run stops, no further item is processed, and `result.Installed`-style counters do not credit a run that ended — true for every (A) site including `adopt.go`, which re-review round 2's NEW-1 fix closed: `ApplyAdopt`'s loop had rendered the completing profile write's own cancellation as an ordinary per-mod `AdoptFailed` and kept going, so a Ctrl-C on the last match exited 0 with the fully-committed mod counted as failed rather than adopted. A non-cancellation failure keeps every site's existing warning/note text byte-for-byte. `profile_apply.go:554` and `profile_sync.go:263/:280/:295` deliberately keep Task 18's plain re-check instead: their #294 warning is the user-visible artifact of the refusal and pinning it is what Important 1's new test does. **(B) Pre-write reads/creates** — `ensureProfileExists` returns any error that is not `domain.ErrProfileNotFound` (`errors.Is`, never `==`) instead of mapping "I could not tell" onto "the profile is fine" (task 18 re-review round 2 NEW-10: this reaches an unreadable-but-present profile YAML too, not just a cancellation - a parse error or EACCES on an install now fails loud instead of silently completing, so a batch install into such a profile aborts up front with a fatal "could not create profile: …" instead of running to completion and reporting success with per-mod warnings, and the single-mod/import call sites gain a "Warning: could not create profile: …" line ahead of the pre-existing "could not update profile" warning; accepted as the correct behavior - nothing should be recorded into a profile that will not load); `import_archive.go:350` now calls that same helper rather than restating it with `==`; `verify_repair.go`'s sibling sweep stops and returns `ctx.Err()` (a third return value, non-nil only under cancellation) instead of emitting N identical bogus per-sibling failures. **(C) Tolerant gates** — `install.go:432`/`:913`/`:1675`, `update.go:808`, `rollback.go:300`, `updater.go:237`/`:282`, `hooks_resolve.go:42`, `queries.go:234`/`:241` return `ctx.Err()` when the profile read failed AND the ctx is cancelled; every other read failure keeps today's tolerant "no lock"/"no profiles" answer. Two signatures moved to make (C) expressible: `Service.Status` gains `(*StatusReport, error)` (it was documented "deliberately errorless", which left no way to say "cancelled" while it reported "no profiles" for every game), and `verifyRun.repairSiblingProfiles` gains its third return. Tests are one per SHAPE, not per site: `TestService_PurgeProfile_CancellationBetweenRecordDeleteAndProfileRefRemoval` (A), `TestEnsureProfileExists_CancelledRead_IsReportedNotTreatedAsExisting` (B), `TestLockedInstallRefusal_CancelledRead_DoesNotDegradeTheGateOpen` (C), plus `TestDoProfileApply_CancellationInsideUpsertMod_IsFatalNotWarned` (review Important 1, pinning the apply guard the same commit had left dead) and `TestProfileManager_Mutators_HonourCancellation` (Minor 4). **Addendum (2026-08-30, Unit R final review Minor 3):** every one of these cancellations still exited 2 with nothing on stdout or stderr naming it, even though `lmm --help` documents 2 as "cancelled by the user" - `Execute`'s exit-2 branch (`root.go`) now prints `Cancelled.` to stderr in plain mode via `printCancelledNotice`; `--json` stays silent, matching Ruling 15's no-extra-text-alongside-a-JSON-contract convention. `TestPrintCancelledNotice_PlainVsJSON` pins both modes. | Supersedes the Ruling 11 row's "left as a follow-up": threading ctx into `ProfileManager` turned "profile write failed" into "possibly a cancellation" at 24 sites, seven of which then returned **exit 0** with a mod in the database, deployed, and absent from its profile YAML — a regression that commit created, invisible to every capture and golden in the repo (which is why deferring it would have meant it never got budgeted). The (A) shape follows Phase 1's Ctrl-C-mid-reinstall lesson: completion and recovery never inherit cancellation, but the cancellation is reported, not swallowed. Also closed here: review Minor 6 (`runListProfiles` takes ctx instead of reading a possibly-nil `cmd.Context()`, and the five test fixtures drop the `SetContext` workaround), Minor 7 (`errors.Is` at `import_archive.go:351`), Minor 5 and Minor 8 (`ProfileManager`'s type doc states the ctx contract, names `ParseProfile` as the exception, and stops pointing at the dissolved `flows.go`) |
| 2026-08-30 | Phase 3 close wave, Important 3(a): `domain.Mod.UpdatedAt` gains `omitzero` - a source that supplies no timestamp now omits `updated_at` from the wire instead of emitting the zero-value `0001-01-01T00:00:00Z`, matching the not-applicable-is-absent convention `convert_paks`/`auth`/`cache_path` already use. 21 `cmd/lmm` goldens re-recorded once (`list_populated`, `search_hits`, `search_capped`, `mod_show_installed`, `mod_show_not_installed`, `mod_show_golden/json_installed.txt`, `mod_lock_result`, `mod_unlock_result`, `mod_set_update_result`, `mod_convert_result`, `mod_edit_result`, `mod_files_report`, `import_archive_result`, `import_scan_dry_run`, `uninstall_dry_run`, `uninstall_dry_run_compile`, `purge_dry_run`, `purge_dry_run_compile`, `profile_apply_dry_run`, `profile_switch_dry_run`, `update_bulk`) — only the `updated_at` key drops (plus, where it was the trailing field, the now-unneeded comma after `dependencies`); no other field moved. No `internal/core`/`internal/domain` golden was affected — none of their fixtures pin a zero `UpdatedAt` | Four consecutive units (O, P, Q, R) deferred the decision and the phase-end review found the zero time still live on the wire in `list`/`search`/`mod show`/`purge --dry-run --json`; ratifying it before the v2.0.0 tag closes the JSON window while it is still open (Unit O M2: "deferring means `0001-01-01T00:00:00Z` is the v2 contract for the whole line") |
| 2026-08-30 | Phase 3 close wave, Minor 1 (Ruling 10 addendum): the last 10 uncited exported `*Service` methods each get a doc sentence naming Ruling 10 and their reason for staying exported - `CheckMergedPakStaleness`, `GetDependencies`, `GetDeployedFilesForMod`, `GetFileOwner`, `GetFilesWithChecksums`, `GetGameCachePath`, `GetInstalledModsInProfileOrder`, `GlobalCacheDir`, `NewUpdater`, `PakNeedsReingest`. All 10 are read-only queries with at least one internal `internal/core` caller (deploy, verify, merged-pak, updater, converge, install, conflicts, `queries.go`) but no `cmd`/`app` caller, so none qualified as "genuinely test-only"; each is kept as a serve-facing query rather than unexported | Closes Unit R Minor 1 / the phase-end review's last inventory gap before the exported-`*Service` surface freezes at v2.0.0 - every export is now a flow, a legitimate query, or a documented test-seed API, with no unlabeled survivor left |
| 2026-08-30 | Phase 3 close wave, Important 3(b): `SearchReport.AttemptedCount`'s `-1` "not applicable" sentinel (a single named `--source` search, which resolves to exactly one attempted source or fails outright and so has no per-source count to report) is ratified as-is — no code change | Unit N M5 and Unit O M3 both asked only that it be decided and recorded, not necessarily changed; `queries.go`'s `SearchReport` doc comment already states the sentinel's meaning precisely, and `-1` cannot collide with a genuine attempted-count of zero (every configured source lacking search capability) |
| 2026-08-30 | v2 pre-cut addendum, Task A (#309), Ruling 17: `lmm game show-default`'s plain-text lines move from stderr to stdout. `cmd.Println`/`cmd.Printf` write to `Command.OutOrStderr()` when no output writer is set on the command (a cobra default every other command already worked around via `cmd.OutOrStdout()` or a direct `fmt.Fprint*(os.Stdout, …)`); `runGameShowDefault` was the one holdout, discovered while giving it a `core.DefaultGame` document for `--json` (an empty document under `--json` on a command whose plain text lands on the wrong stream would otherwise persist). `doGameShowDefault` now writes via `cmd.OutOrStdout()`; the bytes are unchanged, only the stream. `TestGameShowDefault_PlainTextIsOnStdoutNotStderr` pins it with separate stdout/stderr buffers (the two pre-existing tests point both at the same buffer, which cannot tell the streams apart) | Closes the `--json` gap #309 filed at the Phase 3 close (`lmm profile list`/`auth status`/`profile export`/`source validate`/`game show-default` all ignored `--json`); `game show-default`'s stderr accident was the one delta among the five that changes plain-text bytes' DESTINATION, not their content, so it earns its own ruling number rather than folding into R-A5's document-shape description |
| 2026-08-30 | **Ruling 18** (pre-cut #314, recorded plain-text delta): `lmm import <archive>`'s readout prints ONCE per user-level import, and the ID it prints is the ID persisted. `doImport` now plans (`PlanImportArchive`), renders the readout from the plan, answers the conflict question from `plan.Conflicts`, and calls `ApplyImportArchive` once. Under Ruling 7 an accepted conflict re-ran `ImportArchive`, so the readout printed a SECOND time between the prompt and `Deploying to game directory...` - exactly `"\nFetching metadata from acme-source...\n\nMod: Second Mod\n  Source: acme-source\n  ID: B1\n  Version: 1.0\n  Files: 1\n"` in the `cmd/lmm/import_characterize_test.go` capture (const `importReadoutRerunLines`, retired; its bytes are preserved in that file's comment) - and, for an UNLINKED archive, the re-run minted a second uuid, so the ID the user saw was not the ID saved. Capture re-pinned once. The `Fetching metadata from <source>...` progress line moves to `cmd` (printed immediately before `PlanImportArchive`, the work it announces), gated by the predicate core exports as `ImportEnrichmentRuns`, so its bytes and ordering are unchanged. **Addendum (2026-08-30, #314 review I1, recorded plain-text delta):** the readout rendering from the plan before `ApplyImportArchive` runs at all moves one more line relative to it - the `-v` `Warning: could not rename cache entry: …` note, raised inside Apply's enrichment-rename step, used to print BEFORE the readout and now prints AFTER it. Old order: `Fetching metadata…` / `Warning: could not rename cache entry: …` / `Mod: …` / `Warning: could not check conflicts: …`. New order: `Fetching metadata…` / `Mod: …` / `Warning: could not rename cache entry: …` / `Warning: could not check conflicts: …`. `TestDoImport_VerboseCacheRenameFailure_CascadesToDeploymentFailure` now pins the four lines' relative order with index comparisons instead of the order-blind `assert.Contains` it previously used | Ruling 7's second readout and second identity were both artefacts of "archive import cannot be planned", which #314 removes: the plan LISTS the archive instead of ingesting it, so the decision point can be answered before any mutation |
| 2026-08-30 | Pre-cut #314, R-B1 addendum (coordinator ruling): `ImportArchivePlan` carries `Warnings []string` (never nil) beyond the ruled field list. R-B2 moves the metadata enrichment into the plan, which is where its three diagnostics now arise (`source %s is not configured for this game; skipping metadata fetch`, `could not fetch metadata: %v`, `could not resolve source file for archive: %v`); with no carrier two of them would be lost. `cmd` prints them as `Warning: %s` on stderr straight after the plan returns, in today's order; `ApplyImportArchive` copies them into `ImportArchiveResult.Warnings` ahead of its own so a Result-only consumer still sees them. Plans still take no `EventSink` - that convention stands, since `serve` treats plans as reads | Losing a warning silently is a worse delta than one added wire field, and a sink on a Plan would make every other Plan's read-only contract negotiable |
| 2026-08-30 | Pre-cut #314: `--force` keeps skipping the conflict CHECK in `ApplyImportArchive` (`if !opts.Force` wraps the whole gate), so R-B3's plan-vs-ingest conflict-set comparison does not run under `--force` either. `PlanImportArchive` still computes `Conflicts` under `--force` - a preview states what WOULD be overwritten | The "`--force` must skip the conflict check entirely, not just the prompt" promise is pinned by both `cmd` and `core` captures, and running the check under `--force` would resurrect the `-v` `could not check conflicts:` note on a forced import whose entry is missing |
| 2026-08-30 | Pre-cut #314: `ImportArchivePlan.Files` is SORTED, while `Cache.ListFiles` returns `filepath.WalkDir`'s DFS order (`a.esp`, `b/c.txt`, `b.txt` vs `a.esp`, `b.txt`, `b/c.txt`), so R-B5's `plan.Files == ListFiles` property is asserted as sorted-vs-sorted equality | A sorted wire list is the contract R-B1 states; reproducing WalkDir's traversal order in a pure function would be fragile and would pin an implementation detail of the cache walker |
| 2026-08-30 | **Ruling 18, second half** (pre-cut #314, recorded plain-text delta, coordinator ruling): moving the archive-ingestibility checks into `PlanImportArchive` drops the `import failed: ` prefix they used to carry (and, for a member rejection, `extracting archive: ` on top of it). Five old→new lines, verbatim: (1) `import failed: unsupported archive format: .txt` → `unsupported archive format: .txt`; (2) `import failed: <compile-source resolver error>` → `<compile-source resolver error>`; (3) `import failed: validating Bear_Mount.exmodz: <err>` → `validating Bear_Mount.exmodz: <err>`; (4) `import failed: extracting archive: reserved name detected: .lmm-file-7 (archive members may not use lmm's reserved ".lmm-" prefix)` → `reserved name detected: .lmm-file-7 (archive members may not use lmm's reserved ".lmm-" prefix)`, and the same drop for `path traversal detected: <name>`; (5) `import failed: extracting archive: 7z command not found: install p7zip-full to extract .7z and .rar files` → `7z command not found: install p7zip-full to extract .7z and .rar files`. One genuinely NEW class with no old counterpart: `7z listing failed: <err>` (and `7z listing timed out after 5m0s`). All five still exit non-zero and render the `{"error", "details"}` envelope under `--json`. The new wording is pinned byte-exactly by `TestPlanImportArchive_ErrorWording` (unsupported format, reserved name, zip slip) and the exit/envelope contract by `TestDoImport_ArchiveUnsupportedFormat_FailsAndRendersTheEnvelope`. `ApplyImportArchive` keeps `import failed: ` for failures of the ingest it actually performs; both methods' doc comments state which layer owns which prefix | The prefixes were artefacts of the check living inside the ingest; a plan that never extracts must not claim to be extracting |

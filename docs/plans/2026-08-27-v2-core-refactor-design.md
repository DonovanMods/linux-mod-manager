...
| 2026-08-29 | Ruling 5 (#294): the CLASS of swallowed `UpsertMod` failures becomes a non-verbose `Warning:` line - a refused/failed profile write, today a lock refusal but also any profile load/save failure (e.g. `ErrProfileNotFound`), widened from lock-only in the Task 13 review's round 1 fix wave - and not just the two originally-named sites: `profile apply`/`sync` at ruling time, `profile switch` joined in Task 13b; the four hand-worded refusals in `update.go`/`mod_edit.go` print `plan.Refusal`/`core.LockedRefRefusalError` text | A silently dropped refusal read as success |

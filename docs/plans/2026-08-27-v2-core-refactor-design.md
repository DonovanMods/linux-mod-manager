...
| 2026-08-29 | Ruling 5 (#294): the CLASS of swallowed `UpsertMod` lock refusals becomes a non-verbose `Warning:` line, not just the two originally-named sites - `profile apply`/`sync` at ruling time, `profile switch` joined in Task 13b; the four hand-worded refusals in `update.go`/`mod_edit.go` print `plan.Refusal`/`core.LockedRefRefusalError` text | A silently dropped refusal read as success |

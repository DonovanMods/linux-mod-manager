...
> **Archived (historical).** This is the original, early-stage design document from 2026-01-22 — superseded by how lmm actually shipped. See [README.md](../../../README.md) and [CHANGELOG.md](../../../CHANGELOG.md) for current, accurate documentation. Notably, the OAuth authentication flow, the Settings TUI screen, and the `games.yaml` schema described below were never implemented as written; the shipped equivalents (API-key auth, in-app `?` help, and the current `games.yaml` format) differ from this document. Kept for historical reference only.

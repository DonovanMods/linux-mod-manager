...
func TestPurge_ANonActiveProfileClearsOnlyWhatItRecorded(t *testing.T) {
	...
	t.Run("the plan says so", func(t *testing.T) {
		...
		require.Len(t, plan.Mods, 2, "bonly's file goes, and shared's record: a still tracks shared.esp")
		assert.ElementsMatch(t, []string{"bonly", "shared"}, []string{plan.Mods[0].ID, plan.Mods[1].ID})
	...
	t.Run("the apply removes only that", func(t *testing.T) {
		...
		rows, err := f.svc.GetDeployedFilesForMod(ctx, f.game.ID, "a", "src", "shared")
		require.NoError(t, err)
		assert.Equal(t, []string{"shared.esp"}, rows, "the active profile keeps tracking the path they share")
		rows, err = f.svc.GetDeployedFilesForMod(ctx, f.game.ID, "b", "src", "shared")
		require.NoError(t, err)
		assert.Empty(t, rows, "and b's record of it goes")
	...
	t.Run("a path shared after the plan is still kept", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.FileExists(t, filepath.Join(f.gameDir, "bonly.esp"))
		require.Len(t, result.Skipped, 1, "bonly is not cleared")
		assert.Equal(t, "bonly", result.Skipped[0].ModID)
		assert.Contains(t, result.Kept, core.PurgeKeptPath{Path: "bonly.esp", Reason: core.PurgeKeptRecorded, Profiles: []string{"a"}})
		rows, err := f.svc.GetDeployedFilesForMod(ctx, f.game.ID, "b", "src", "bonly")
		require.NoError(t, err)
		assert.Equal(t, []string{"bonly.esp"}, rows, "a record the plan did not say goes stays")
	})

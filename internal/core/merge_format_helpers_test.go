...
// registerCompileAdapterOnly registers mc as the game adapter WITHOUT
// registering it as a mod source. It is how a test says "the compile
// capability and the source serving the bytes are different things", which
// is exactly what U2 (#412) made expressible.
func registerCompileAdapterOnly(svc *core.Service, mc adapter.MergeCompiler) {
	svc.RegisterAdapter(testCompileAdapter{MergeCompiler: mc})
}

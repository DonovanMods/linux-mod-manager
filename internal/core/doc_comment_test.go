...
// (v2 Phase 3 Task 20, #305): every exported type, func, method (on an
// exported receiver type — a method on an unexported receiver, e.g.
// *reinstallCacheTransaction, is not part of the package's godoc surface
// and is excluded), const, and var declared in this package's non-test
// files must carry a leading doc comment, so `go doc core` (and a future
// pkg.go.dev listing) never has a blank entry. A comment directly above a
// `const (`/`var (` block's opening paren covers every name in that block
// that has no comment of its own (accepted Go style for a run of related
// one-liners, e.g. the Op*/Verify*/UpdateStatus enums) — but each FuncDecl
...
// golang.org/x/tools/go/packages — same approach as
// internal/domain's json_contract_coverage_test.go.

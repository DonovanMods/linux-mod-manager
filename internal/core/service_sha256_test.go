...
func downloadWithSHA256(t *testing.T, content []byte, expectedSHA string) (game *domain.Game, mod *domain.Mod, cached func() bool, err error) {
	t.Helper()
...
func TestDownloadModVerifiesDeclaredSHA256(t *testing.T) {
	...
	t.Run("matching hash passes", func(t *testing.T) {
		_, _, cached, err := downloadWithSHA256(t, content, good)
		require.NoError(t, err)
		assert.True(t, cached())
	})

	t.Run("uppercase hash passes (case-insensitive)", func(t *testing.T) {
		_, _, cached, err := downloadWithSHA256(t, content, strings.ToUpper(good))
		require.NoError(t, err)
		assert.True(t, cached())
	})

	t.Run("mismatched hash fails and nothing is cached", func(t *testing.T) {
		_, _, cached, err := downloadWithSHA256(t, content, strings.Repeat("ab", 32))
		require.Error(t, err)
	...
	t.Run("empty hash skips verification", func(t *testing.T) {
		_, _, cached, err := downloadWithSHA256(t, content, "")
		require.NoError(t, err)
		assert.True(t, cached())
	})
}

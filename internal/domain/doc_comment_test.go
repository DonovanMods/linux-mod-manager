...
// ratchet (v2 Phase 3 Task 20, #305) extended to internal/domain: every
...
// (accepted Go style for a run of related one-liners, e.g. the
// LinkMethod/DeployMode/UpdatePolicy enums) — but each FuncDecl is its own
// declaration, so a comment above the first of a run of one-liner methods
// does NOT cover the rest of the run; each needs its own line.

...
//
// Preact's hooks (useState, useEffect, ...), a separate entry point of the
// same package at the same pinned version.
//
// Package:  preact (hooks entry point)
// Version:  10.29.8 (pinned)
...
//  2. the bare specifier `from"preact"` rewritten to
//     `from"./preact.module.js"`, so the shell needs no
//     <script type="importmap"> - which would be a SECOND inline script,
//     and the theme bootstrap is the only one this page is allowed
//
// To re-verify: strip the header block above (and undo the edits) and

...
// TestSameFileAtPath_ALockFileUnlinkedUnderItsHolderIsNotTheLock is the
// check that closes the race a prune introduces (#410): a waiter that wins
// the flock on a lock file which has since been removed - what a prune does
// while holding it - holds nothing a newcomer can see, because the newcomer
// opens a NEW file at the same path. lockCommunity drops such a lock and
// takes the one at the path instead; this pins the test it decides on.
func TestSameFileAtPath_ALockFileUnlinkedUnderItsHolderIsNotTheLock(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, lockFileName)
	held, err := os.OpenFile(path, os.O_RDWR|os.O_CREATE, 0o600)
	require.NoError(t, err)
	defer func() { _ = held.Close() }()
	assert.True(t, sameFileAtPath(held, path), "the file just opened is the one at the path")

	require.NoError(t, os.Remove(path))
	assert.False(t, sameFileAtPath(held, path), "a removed file is nobody's lock")

	replacement, err := os.OpenFile(path, os.O_RDWR|os.O_CREATE, 0o600)
	require.NoError(t, err)
	defer func() { _ = replacement.Close() }()
	assert.False(t, sameFileAtPath(held, path), "nor is it once another file has taken the name")
	assert.True(t, sameFileAtPath(replacement, path))
}

...
func TestViewFitsTerminalBoundsWithHelpVisible(t *testing.T) {
	...
	// Height bumped 37->39->40->60->61->65->66->68->69 over time (37->39->40
	...
	// Rollback entry added one more line to that same group - see
	// helpGroups' own conflicts group doc comment. Verified empirically each
	// time (scratch probes sweeping a height range, since removed) the same
	// way 5a proved its own 36->37 bump: below the fitting height, the
	...
	// before the requested budget does. Height=69 is the first value where
	// the requested content budget finally reaches that same natural
	// minimum, so the view fits with exactly zero slack (70 and above, the
	// content grows to fill the larger budget instead; 66-68 still overflow
	// - the probe showed h=66→68, h=67→68, and h=68→69 rendered heights).
	// This pins the current zero-slack floor - see task-5-brief.md's "prove
	// pre-existing saturation... like 5a did" allowance for justified height
	// adjustments.
	model := sizedPrototypeModel(t, "wizardry", 120, 69)
	model = updateWithRunes(t, model, "?")

	view := model.View()
	require.Equal(t, 120, lipgloss.Width(view))
	require.Equal(t, 69, lipgloss.Height(view))
}

...
EVERY installed Steam game: the extra rows carry `known: false`, no
sources, an empty `mod_path` and `index: 0` — they are listed, not
...
| `lmm game add --from-detected <steam-app-id>`             | Prefill everything from an installed Steam game — name, install path, game id, mod path, and a known game's source map; every other flag still wins                                  |
...
| `lmm game detect --include-unknown`                       | Also list installed games with no known-games entry, with their Steam app ids for `lmm game add --from-detected`; under `--json` (with no selection flag) emits the listing document |

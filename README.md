...
**When Thunderstore is slow or refusing.** Thunderstore publishes no rate limit for the endpoints lmm uses, but a busy edge can still answer `429`. lmm retries twice, waiting at least what the server's `Retry-After` asks (up to a minute), and says so as it waits — `Rate limited by Thunderstore; retrying in 12s (attempt 2 of 3).` on stderr, or as the progress text of the web UI job that is waiting. A longer requested wait, or three failed requests in a row, holds Thunderstore off for a while; lmm names when it will ask again (`Not asking Thunderstore again until 12:04:30 (in 4m30s).`), and a search in the meantime answers from the copy on disk if there is one. A transfer that stops delivering bytes fails as stalled after 30 seconds (a minute for a mod download) rather than waiting out the ten-minute ceiling a large index is allowed.

...
A prune removes an index no game in `games.yaml` maps, and a mapped one only once it has gone 30 days without a refresh; `--all` removes every index, after asking. It is fail-closed: nothing is removed if `games.yaml` cannot be read (and only `--all` removes anything if there is no `games.yaml` at all), no unused index is removed while any game maps Thunderstore to an identifier lmm cannot use, an index refreshed after the prune decided to remove it is kept, and a directory is removed only when it holds nothing but the index files lmm wrote there — never through a symbolic link, and not at all under a symlinked `_thunderstore` directory. Everything removed is rebuilt by the next search. `--json` prints the same documents the web UI reads.

In the web UI, the **Setup → Sources** page lists the same indexes with a **Refresh index** button for each one a game uses and a **Prune unused indexes** action that previews first and removes exactly what it showed; the search page says "Building the Thunderstore index for … (one-time)…" while a first search waits on the build.

**When Thunderstore is slow or refusing.** Thunderstore publishes no rate limit for the endpoints lmm uses, but a busy edge can still answer `429`. lmm retries twice, waiting at least what the server's `Retry-After` asks (up to a minute), and says so as it waits — `Rate limited by Thunderstore; retrying in 12s (attempt 2 of 3).` on stderr, or as the progress text of the web UI job that is waiting. A longer requested wait (or one named on the last attempt), or three failed requests in a row, holds Thunderstore off for a while; lmm names when it will ask again (`Not asking Thunderstore again until 12:04:30 (in 4m30s).`), and a search in the meantime answers from the copy on disk if there is one. A mod download told to wait longer than a minute fails at once, naming the wait. A transfer that stops delivering bytes fails as stalled after 30 seconds (a minute for a mod download) rather than waiting out the ten-minute ceiling a large index is allowed.

...
└── serve/                # `lmm serve` local web UI (flat package, 29 files); imports only app/core/domain (boundary_test.go)
    ...
    ├── routes.go          # Registers every page, /api/v1, and /static/ route on the mux
    ├── render.go          # Executes the shared "layout" template, writes the response
    ├── selection.go       # ?game/?profile resolution shared by every scoped page + its API mirror
    ├── static.go          # go:embed static/app.css + static/app.js
    ├── templates.go       # go:embed templates/*.gohtml
    ├── pages_status.go    # `/` status dashboard
    ├── pages_mods.go      # `/mods` list + `/mods/{source}/{id}` details (#232, changelog #87)
    ├── pages_search.go    # `/search` + install form
    ├── pages_updates.go   # `/updates` per-item update checkboxes (#74)
    ├── pages_profiles.go  # `/profiles` list + switch/apply/deploy forms
    ├── pages_health.go    # `/health` verify findings + conflicts
    ├── pages_jobs.go      # `/jobs/{id}` live job page (progress; result when done)
    ...
    ├── mutations.go       # Shared mutation request parsing, confirm-page chrome, recovery-form fields, `?sync=1`
    ├── plankinds.go       # Closed table of mutation "kind" strings the job API accepts
    ├── kind_toggle.go     # enable/disable - the one sanctioned non-Plan job path
...
| Serve (HTTP)         | `httptest` against a `Service` seeded with the existing fake-source fixtures, sandboxed HOME/XDG as everywhere else; pages and `/api/v1` are asserted semantically (status, form targets/methods, CSRF token presence, decoded JSON) — never byte-golden HTML; every mutation test exercises both the plain-POST (no-JS) path and its `?sync=1` inline variant; SSE framing/replay/heartbeat use a fake-clock seam; the job registry's ring buffer and eviction are `-race`-sensitive and run under it. Two more go/ast ratchets: `no_unsafe_html_test.go`'s `TestNoUnsafeTemplateCasts` forbids `template.HTML`/`template.JS` escape-hatch casts anywhere in the package, and `css_coverage_internal_test.go`'s `TestAppCSS_CoversEveryTemplateClass` fails the build if a template references a Tailwind class `static/app.css` doesn't define                                                                                                                                                                                                                                         |

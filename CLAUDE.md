...
└── serve/                # `lmm serve` local web UI: an SPA over /api/v1; imports only app/core/domain (boundary_test.go)
    ...
    ├── spa/               # index.html shell + app.css (Launcher tokens, dark+light) + app/*.js (ES modules: render/router/store/api/sse/theme/main, activity+progress+failures for the job layer, modrows/navigation/ui, + components/: app, missioncontrol, topbar, cards, library, gamechooser, modpanel, fullmodpage, searchpage, omnibarresults, searchresults, tray, modal, confirmplan, planrenderers, plan_deploy, plan_install, plan_uninstall, plan_rollback, documentview, jobprogress, toasts)
    ...
    ├── kind_switch.go     # `lmm profile switch` as a job
    ├── kind_profile_apply.go # `lmm profile apply` as a job
